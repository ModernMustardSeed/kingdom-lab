...
const GoldStardust: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      // Classes defined inside sketch to access p5 instance 'p' easily
      ...
      class BackgroundStar {
        ...
        constructor() {
          ...
          // Mostly white/cream, occasional warm tint
      ...
      class ShootingStar {
        ...
        constructor() {
          // Start mostly from top or left side to streak across
          if (p.random() > 0.5) {
          ...
          // Angle generally downwards-right (PI/4 is diagonal down-right)
        ...
        display() {
          ...
          const steps = 20; // Number of segments in the tail
          ...
          for (let i = 0; i < steps; i++) {
            ...
            // Tail fades out and gets smaller
      ...
      class Particle {
        ...
        constructor(index: number) {
          this.index = index;
          // Initialize properties to satisfy TS, then call reset
        ...
        reset(randomY = false) {
          this.pos = p.createVector(
            p.random(p.width),
            randomY ? p.random(p.height) : p.random(-100, -20)
          );
          this.vel = p.createVector(0, 0);
          this.acc = p.createVector(0, 0);
          ...
          this.depth = p.random();
          this.depth = p.pow(this.depth, 0.7); 
        ...
        display() {
          ...
          this.drawTrail(col, brightness, alphaBoost);
        ...
        drawTrail(col: ColorHSB, brightness: number, alphaBoost: number) {
      ...
      p.draw = () => {
        ...
        if (p.random() < 0.005) { // Chance to spawn a shooting star
